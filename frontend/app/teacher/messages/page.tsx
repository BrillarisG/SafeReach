...
type Conversation = { id: string; name: string; sub: string; time: string; unread: number; avatar: string; online?: boolean; studentId?: string; group: ConversationGroup };
type Message = { from: string; text: string; time: string; me: boolean };

const baseConversations: Conversation[] = [
  { id: 'base-1', name: 'Rajesh Shah (Parent)', sub: "Re: Aryan's performance this term", time: '10:32 AM', unread: 2, avatar: 'RS', online: true, group: 'direct' },
  ...
  { id: 'base-4', name: 'Mr. David Lee', sub: 'Can we swap supervision slots?', time: 'May 26', unread: 0, avatar: 'DL', online: true, group: 'teacher' },
...
export default function TeacherMessagesPage() {
  ...
  const groupConversations: Conversation[] = [
    { id: 'group-admin', name: 'Admin + All Teachers', sub: 'Common group with school admin and every teacher', time: 'Live', unread: 1, avatar: 'AD', online: true, group: 'admin' },
  ...
  function changeGroup(group: ConversationGroup) {
    ...
    if (group === 'direct') {
      ...
      selectConversation(firstInGroup?.id ?? '');
    }
    setSentNotice('');
  }
  ...
  function sendMessage() {
    ...
    setRecentActivity(current => ({ ...current, [activeConv.id]: Date.now() }));
    ...
    <div className="flex h-[calc(100dvh-4rem)] w-full min-w-0 max-w-full min-h-0 overflow-hidden">
      <aside className="w-80 border-r border-outline-variant/30 bg-surface-container-low flex-col shrink-0 hidden md:flex">
        ...
        <div className="order-3 flex-1 overflow-y-auto">
          {filteredConversations.map(c => (
            <button key={c.id} onClick={() => selectConversation(c.id)} className={`w-full flex items-center gap-3 px-4 py-3 border-b border-outline-variant/20 text-left transition-colors ${active === c.id ? 'bg-primary-container/30' : 'hover:bg-surface-container'}`}>
              <div className="relative shrink-0"><div className={`w-10 h-10 rounded-full flex items-center justify-center text-label-sm font-bold ${active === c.id ? 'bg-primary text-white' : 'bg-primary-container text-primary'}`}>{c.avatar}</div>{c.online && <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white bg-green-500" />}</div>
      ...
      <div className={`${mobileChatOpen ? 'hidden' : 'flex'} md:hidden w-full min-w-0 flex-1 flex-col bg-background`}>
        ...
        <div className="flex-1 overflow-y-auto bg-background">
          {filteredConversations.map(conversation => (
            ...
            >
              <div className="relative shrink-0"><div className="w-12 h-12 rounded-full overflow-hidden bg-primary-container text-primary flex items-center justify-center text-label-md font-bold">{conversation.avatar}</div>{conversation.online && <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white bg-green-500" />}</div>
      ...
      <div className={`${mobileChatOpen ? 'flex' : 'hidden'} md:flex w-full min-w-0 min-h-0 flex-1 flex-col bg-background`}>
        <div className="px-4 py-3 bg-surface border-b border-outline-variant/30 flex items-center gap-3 shrink-0">
          ...
          <div className="relative"><div className="w-9 h-9 rounded-full bg-primary text-white flex items-center justify-center font-bold text-label-sm">{activeConv?.avatar}</div>{activeConv?.online && <span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-white bg-green-500" />}</div>
